...
# Load the entire model (model + preprocessing) from the saved pipeline

pipeline = joblib.load("gradient_boosting_model.pkl", mmap_mode=None)
print(pipeline)
...
def predict():
    try:
        ...
        # Convert input data to DataFrame to match model input format
        ...
        # Return the result
        result = {
            "prediction": "✅ Charging Will Likely Complete" if prediction[0] == 1 else "❌ Charging May Not Complete"
        }
